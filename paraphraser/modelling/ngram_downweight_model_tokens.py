...
from paraphraser.modelling.utils import make_vocab_start_map, make_word_penalties_tokens
...
class NgramDownweightDecoder(FairseqDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out):
        ...
        #
        # print("lines in decoder", lines, prev_output_tokens)

        for ii, (toks, penalties) in enumerate(zip(prev_output_tokens, encoder_out)):
            ...
            for n_gram_n in range(1, min(len(toks)+2, max_prefix_len)):
                ...
                if prefix in penalties:
                    ...
                    for prefix_len_in_words in penalties[prefix]:

                        a = self.args.prism_a
                        b = self.args.prism_b
                        p = prefix_len_in_words + 1
                        # if prefix == tuple():
                        #     p = (ii+1) * p
